...
class ScrapServiceTest {
	...
	void createScrapByCreateRequest_Success() {
		...
		given(scrapMapper.scrapQuestionToScrapQuestionResponse(scrapQuestion1)).willReturn(
			new ScrapQuestionResponse(scrapQuestion1.getScrapQuestionId(), scrapQuestion1.getContent()));
		given(scrapMapper.scrapQuestionToScrapQuestionResponse(scrapQuestion2)).willReturn(
			new ScrapQuestionResponse(scrapQuestion2.getScrapQuestionId(), scrapQuestion2.getContent()));

		//when
		CreateScrapResponse createScrapResponse = scrapService.createScrapByCreateRequest(createScrapRequest, 10);

		//then
		assertThat(createScrapResponse.getOriginalInterview().getInterviewId()).isEqualTo(
			originalInterview.getInterviewId());
		...
		assertThat(createScrapResponse.getScrapQuestions().get(0).getScrapQuestionId()).isEqualTo(
			scrapQuestions.get(0).getScrapQuestionId());
		assertThat(createScrapResponse.getScrapQuestions().get(0).getContent()).isEqualTo(
			scrapQuestions.get(0).getContent());
	}
