...
public class NaverOAuthService {
	...
	public Account naverLoginOrRegister(String authorizationCode, String state) throws JsonProcessingException {
		...
		ResponseEntity<String> tokenResponseJson = restTemplate.postForEntity(naverTokenUri, httpRequestEntity,
			String.class);
		ObjectMapper objectMapper = new ObjectMapper();
		objectMapper.setPropertyNamingStrategy(PropertyNamingStrategy.SNAKE_CASE);
		objectMapper.setSerializationInclusion(JsonInclude.Include.NON_NULL);
		NaverTokenResponse naverTokenResponse = objectMapper.readValue(tokenResponseJson.getBody(),
		...
		NaverUserInfoResponse naverUserInfoResponse = objectMapper.readValue(tokenResponseJson.getBody(),
			new TypeReference<NaverUserInfoResponse>() {
			});
